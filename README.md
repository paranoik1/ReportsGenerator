...
> 
> ### 🔧 Пользовательская конфигурация моделей
>
> Каждый агент может использовать свою модель и подключение. В веб-интерфейсе доступна секция **«Настройки AI моделей»**, где можно указать для каждого агента:
> 
> - **Модель** — название модели (например, `gpt-4o`, `qwen3.5:cloud`, `kimi-k2-thinking:cloud`)
> - **Base URL** — адрес API (например, `http://127.0.0.1:11434/v1`)
> - **API Key** — ключ для аутентификации
>
> Если поля оставить пустыми — будут использоваться глобальные настройки из конфигурации.
...
| 🔹 **Настраиваемые AI-модели** | Пользователи могут указать свои модели, API-ключи и base_url для каждого агента через веб-интерфейс |
...
### 🔧 Создание задачи с кастомными моделями
```bash
curl -X POST http://127.0.0.1:5000/start \
  -F "prompt=Составь отчёт по практической работе" \
  -F "files=@source.pdf" \
...
```
Доступные префиксы параметров: `model_`, `base_url_`, `api_key_` для каждого агента:
- `document_analyst`
- `template_analyst`
- `user_prompt_analyst`
- `formatter`
