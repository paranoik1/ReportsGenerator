...
from openai.types import ReasoningEffort, Reasoning
...
class AgentState:
    ...
    models_pipeline: list[ModelPipeline] = field(default_factory=list)

    current_step: int = 0
    iteration: int = 0
    max_iterations: int = 10
    finished: bool = False
    # documents_summary: list[DocumentSummary] = field(default_factory=list)
    documents: list[Document] = field(default_factory=list)
    
    # Для Human-in-the-loop
    pending_approval: CodeResult | None = None
    working_dir: str = field(default_factory=lambda: tempfile.mkdtemp(prefix="reports_gen_"))
...
class Orchestrator:
    ...
    def __init__(self, base_url: str = "http://127.0.0.1:11434/v1", api_key: str = "ollama") -> None:
        self.client = OpenAI(
            base_url=base_url,
            api_key=api_key
        )
    ...
    def supervisor_agent(self, state: AgentState) -> list[Step]:
        ...
        docs_info_map = map(lambda doc: doc.filepath + "\n" + doc.raw_text, state.documents)
        docs_context = "\n\n".join(docs_info_map)

        # docs_context = "\n".join(
        #     f"{d.title}: {d.summary}" for d in state.documents_summary
        # )
        ...
        pipeline = ModelPipeline(model, messages)

        response = self.run_agent(model, messages)
        # response = self.client.responses.parse(
        #     text_format=Step,
        #     model=model.name,
        #     input=pipeline.context, # type: ignore
        #     tools=model.tools, # type: ignore
        #     reasoning=Reasoning(effort=model.reasoning_effort), # type: ignore
        #     temperature=model.temperature,
        # )

        # Парсим ответ как JSON со списком шагов
        try:
            steps_data = json.loads(response.output[0].content) if hasattr(response, 'output') else json.loads(response)
    ...
    def coder_agent(self, state: AgentState, step: Step) -> CodeResult:
        ...
        # Собираем контекст
        codes_context = "\n\n".join(
            cr.code for cr in state.generated_codes
        )
...
"""
    ...
    def execute_code(self, code: str, language: str = "python", use_docker: bool = False) -> CodeResult:
        """
    ...
    def diagram_creator_agent(self, state: AgentState, step: Step) -> DiagramResult:
        ...
        # Собираем контекст
        diagrams_context = "\n\n".join(
            d.code for d in state.diagrams
        )
...
"""
    ...
    def formatter_agent(self, state: AgentState) -> str:
        ...
        docs_context = "\n\n".join(
            doc.raw_text for doc in state.documents
        )
...
"""
    ...
    def approve_code(self, state: AgentState, approved: bool, edited_code: str | None = None) -> CodeResult:
        """
